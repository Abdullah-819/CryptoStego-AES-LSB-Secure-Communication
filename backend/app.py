...
from security_module import encrypt_message, decrypt_message, embed_data_into_image, extract_data_from_image, visualize_lsb_changes, calculate_entropy
...
def decode():
    try:
        ...
        if extracted_data is None:
            return jsonify({'error': 'No hidden data detected'}), 404

        decrypted_message = decrypt_message(extracted_data, password)
        if decrypted_message is None:
            return jsonify({'error': 'Incorrect password or corrupted data'}), 401
